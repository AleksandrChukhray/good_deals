...
<form id="ad_tov" method="post" action="/market/new_tovar_gift/" enctype="multipart/form-data" >

    <div id="left_add">
        <span class="headadd">Объявление</span><br>
        <div class="hr"></div><br>
        <input type="radio" id="sell" name="sel_bay" value="g" required><label for="sell" ><i><i></i></i></label><span class="headadd">Дарю</span>
        <input type="radio" id="bay" name="sel_bay" value="c" required><label for="bay" style="margin-left: 30%"><i><i></i></i></label><span class="headadd">Меняю</span><br>
        ...
        <label for="desc_sh">Описание <span style="color:red">*</span></label><textarea name="desc_sh" id="desc_sh" style="margin-left:14% !important" required></textarea><br>
        <label for="full_sh">Варианты обмена<span style="color:red">*</span></label><textarea name="full_sh" id="full_sh" style="margin-left:3% !important" required></textarea><br>
        <label for="cost">Условия обмена</label><textarea name="cost" id="cost" style="margin-left:6.5% !important;  width: 48%;height: 100px;border-radius: 4px;margin-top: 5%;"></textarea><br><br>
    ...
    <div id="right_add">
        ...
        <div id="groups" style="padding:15px 30px 15px 30px; background-color: rgba(223, 192, 255, 0.6); text-align: left !important; border-radius:6px;">
            ...
            <div style="float:left;"> <input type="radio" value="unreg" name="termin" id="unreg" checked="checked"><label for="unreg"><i><i></i></i></label><span class="headadd">1 месяц</span><br> 
                <input type="radio" value="2reg" name="termin" id="2reg"  <?php
                if (!isset($_SESSION['auth'])) {
                    echo "disabled";
                }
                ?>  ><label for="2reg"><i><i></i></i></label><span class="headadd">2 месяца <?php
                       if (!isset($_SESSION['auth'])) {
                           echo "<span style='font-size:8px !important;'> (для зарегистрированых)</span>";
                       }
                ?></span><br>
            </div>
            <div > <input type="radio" value="3reg" name="termin" id="3reg"  <?php
                if (!isset($_SESSION['auth'])) {
                    echo "disabled";
                }
                ?>  ><label for="3reg"><i><i></i></i></label><span class="headadd">3 месяца <?php
                                   if (!isset($_SESSION['auth'])) {
                                       echo "<span style='font-size:8px !important;'> (для зарегистрированых)</span>";
                                   }
                                   ?></span><br>
                <input type="radio" value="reg"  name="termin" id="reg"   <?php
                                   if (!isset($_SESSION['auth'])) {
                                       echo "disabled";
                                   }
                                   ?>  ><label for="reg"><i><i></i></i></label><span class="headadd">6 месяцев <?php
                                   if (!isset($_SESSION['auth'])) {
                                       echo "<span style='font-size:8px !important;'> (для зарегистрированых)</span>";
                                   }
                                   ?></span>
        ...
        <a href="/market/gift_product" style="color:grey;text-decoration:underline;margin-right: 25%">отмена</a>
    </div>
</form>
